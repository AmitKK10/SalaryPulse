...
export default function LiveWork() {
  ...
  const { state, start, pause, resume, end } = useLiveWork();
  useTicker(state.status === 'running');

  const live = liveEarnings(state.startedAt, state.pausedMs, settings, state.pausedAt);
  const progress = {
    progress: settings.requiredDailyHours ? (live.activeHours / settings.requiredDailyHours) * 100 : 100,
    workedHours: live.activeHours,
    adjustedTargetHours: settings.requiredDailyHours,
    remainingHours: live.remainingHours,
  ...
  return (
    <>
      ...
      <Panel>
        <div className="flex flex-col gap-5 text-center sm:flex-row sm:items-center sm:justify-between sm:text-left">
          <div>
            ...
            <h2 className="mt-3 text-4xl font-black">{hoursMinutes(live.activeHours)}</h2>
            <p className="mt-2 text-slate-400 light:text-slate-500">Active time today</p>
          </div>
          <div className="sm:text-right">
            <p className="text-sm text-slate-400 light:text-slate-500">Live salary</p>
            <p className="mt-2 text-4xl font-black text-mint">{currency(live.earnings, settings)}</p>
          </div>
        </div>
      ...
      <section className="mt-4">
        <WorkProgressCard title="Today Work Progress" progress={progress} live={state.status === 'running'} />
      </section>
      <section className="mt-4 grid gap-3 sm:grid-cols-3">
        ...
        <StatCard label="Daily target progress" value={percent(progress.progress)} helper={currency(live.earnings, settings)} />
      </section>
    </>
  );
}
